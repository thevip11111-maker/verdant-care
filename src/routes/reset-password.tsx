...
function ResetPassword() {
  ...
  return <main className="grid min-h-screen place-items-center bg-surface px-5"><section className="w-full max-w-md rounded-xl border border-border bg-card p-6 shadow-sm"><span className="grid size-10 place-items-center rounded-xl bg-brand text-brand-foreground"><Leaf/></span><h1 className="mt-6 font-display text-3xl font-extrabold">Choose a new password</h1><p className="mt-2 text-sm text-muted-foreground">Use at least eight characters.</p><form onSubmit={submit} className="mt-6 space-y-4"><Input name="password" type="password" minLength={8} required placeholder="New password"/><Button size="lg" className="w-full">Update password</Button></form>{message&&<p className="mt-4 text-sm" role="status">{message}</p>}<Button variant="ghost" className="mt-2 w-full" onClick={()=>navigate({to:"/"})}>Back to Verdant</Button></section></main>;
}